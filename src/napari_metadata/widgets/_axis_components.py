"""Per-axis metadata editing components (labels, translations, scales, units).

Each component class is decorated with ``@_axis_metadata_component`` to
register it in ``AXIS_METADATA_COMPONENTS_DICT``.  The coordinator class
``AxisMetadata`` instantiates all registered components and exposes them
...
def _axis_metadata_component(
    ...
    """Decorator that registers an ``AxisComponent`` implementation and
    injects shared helper methods (``_reset_tuples``,
    ``_set_axis_name_labels``, ``_set_checkboxes_visibility``).
    """
    ...
    def set_checkboxes_visibility_injection(
        ...
        for checkbox in self._inherit_checkbox_tuple:
...
class AxisLabels:
    ...
    def load_entries(self, layer: Layer | None = None) -> None:
        ...
        if active_layer != self._selected_layer or active_layer is None:
            ...
            return
        layer_labels = get_axes_labels(self._napari_viewer, active_layer)
    ...
    def inherit_layer_properties(self, template_layer: Layer) -> None:
        ...
        checkbox_list = self._inherit_checkbox_tuple
        ...
        set_axes_labels(self._napari_viewer, tuple(setting_labels))
        self._selected_layer = None
...
class AxisTranslations:
    ...
    def inherit_layer_properties(self, template_layer: Layer) -> None:
        ...
        set_axes_translations(self._napari_viewer, tuple(setting_translates))
        self._selected_layer = None
...
class AxisScales:
    ...
    def load_entries(self, layer: Layer | None = None) -> None:
        ...
        layer_scales = get_axes_scales(self._napari_viewer, active_layer)
    ...
    def inherit_layer_properties(self, template_layer: Layer) -> None:
        ...
        set_axes_scales(self._napari_viewer, tuple(setting_scales))
        self._selected_layer = None
...
class AxisUnits:
    ...
    def load_entries(self, layer: Layer | None = None) -> None:
        ...
        layer_units = get_axes_units(self._napari_viewer, active_layer)
    ...
    def _update_units_visibilities(self) -> None:
        for axis_index in range(len(self._axis_name_labels_tuple)):
            type_combobox = self._type_combobox_tuple[axis_index]
            unit_combobox = self._unit_combobox_tuple[axis_index]
    ...
    def _set_unit_combobox(
        ...
        application_reg = pint.get_application_registry()
        with QSignalBlocker(combobox):
            ...
            if found_type is None:
                ...
                setting_pint_unit = application_reg.Unit(unit_type_string)
    ...
    def _set_current_combobox_units_to_layer(self) -> None:
        ...
        for axis_index in range(len(self._axis_name_labels_tuple)):
            ...
            if axis_type is None:
                units_list.append(None)  # type: ignore[arg-type]
                continue
    ...
    def _on_type_combobox_changed(self) -> None:
        current_units: tuple[pint.Unit | None, ...] = get_axes_units(
            self._napari_viewer,
            resolve_layer(self._napari_viewer),
        )
        for axis_number in range(len(self._axis_name_labels_tuple)):
            type_combobox = self._type_combobox_tuple[axis_number]
            unit_combobox = self._unit_combobox_tuple[axis_number]
    ...
    def inherit_layer_properties(self, template_layer: Layer) -> None:
        ...
        set_axes_units(self._napari_viewer, tuple(setting_units))  # type: ignore[arg-type]
...
def _get_axis_label_tuple(
    ...
    for label_index, label in enumerate(axis_labels_tuple):
        ...
        axis_label.setText(label if label else f'{label_index}')
...
def _get_double_spinbox_tuple(
    ...
    if layer is None:
        return ()
    getter_map = {
        'get_axes_translations': get_axes_translations,
        'get_axes_scales': get_axes_scales,
    }
    getter = getter_map.get(obtain_tuple_method_str)
    if getter is None:
        msg = f'{obtain_tuple_method_str!r} is not a valid getter'
        raise KeyError(msg)
    setting_values: tuple[float, ...] = getter(viewer, layer)
...
class AxisMetadata:
    ...
    def __init__(
        ...
        for (
            ...
            self._axis_metadata_components_dict[name] = cls(
    ...
    def _update_axes_labels(self) -> None:
        for component in self._axis_metadata_components_dict.values():
            cast(AxisComponent, component)._set_axis_name_labels()

    def _set_inheritance_checkbox_visibility(self, visible: bool) -> None:
        for component in self._axis_metadata_components_dict.values():
            cast(AxisComponent, component)._set_checkboxes_visibility(visible)
